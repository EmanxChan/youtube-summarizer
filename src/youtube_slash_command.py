    ...
    if ai_summarizer:
        ...
        if cached_summary:
            ...
            if summary:
                ...
                absolute_minimum = 150  # Never fall back if we have at least a decent summary
...
{content_text[:6000]}"""
                    ...
                    if retry_word_count < min_word_count:
                        # Only fall back to extractive if the AI summary is completely inadequate
                        if retry_word_count < absolute_minimum:
                            print(f"⚠ Retry still too short ({retry_word_count} words). Falling back to extractive method...")
                            summary = summarize_transcript(content_text, word_count)
                        else:
                            print(f"✓ Retry produced {retry_word_count} words (below target of {word_count}, but acceptable)")
                    else:
