...
class AITranscriptSummarizer:
    ...
    def _init_ollama(self):
        ...
        try:
            ...
            self.model = self.model or "mistral:instruct"  # Default to Mistral 7B Instruct
            # Test connection
            try:
    ...
    def generate_key_takeaways(self, transcript: str, video_title: str, 
        ...
        Returns:
            List of actionable takeaway strings
        """
        ...
        prompt = f"""You are an expert at extracting actionable insights from video transcripts.
...
Format each takeaway as a complete, standalone sentence that would be valuable even without context.
...
- "Implement test-driven development workflow to catch bugs before they reach production"

Return ONLY the {count} takeaways, one per line, without numbers or bullet points."""
        ...
        try:
            if self.provider in ["openai", "deepseek"]:
                response = self.client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are an expert at creating actionable summaries."},
